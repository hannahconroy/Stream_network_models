...
"Stream Network Model with Dynamic Stream Lines" 
...
The following is the stream network model adapted from the Saccardi and Winnick paper 2021 'Improving predictions of stream CO2 concentrations and fluxes using a stream network model: a case study in the East River Watershed, CO, USA'. The code was adapted for the paper "Seasonality Drives Carbon Emissions along a Stream Network" by Conroy et al. 2023 by Hannah Conroy and then again for the paper "Hydrologic Regimes Shape Stream Network Carbon Dioxide Dynamics and Emissions" by Conroy et al. 2025. The new model uses a TOPMODEL addition to model stream intermittence at five NEON sites acros the United States.  
...
require("knitr")
opts_knit$set(root.dir ='C:/Users/Name/Documents/SN_files')
```
...
site <- "WALK" #options are: MART, KING, CARI, COMO, WALK
run_type <- "regular" #options are: regular, full, lower, and upper, gw_inputs, NEP_upper, NEP_daily
discharge_type <- "measured" 
threshold_run <- "FLASE" #use for threshold runs testing using 3E-4 Q threshold for stream permanence (run_type should be full)
...
#read in top model inputs 
tm_inputs <- read.csv(paste0(site,"/tm_inputs_", site, ".csv"))


SN_inputs <- read.csv(paste0(site, "/input_SN_", site, ".csv"))
#SN_inputs <- SN_inputs[, -which(names(SN_inputs) == "X")]
# Rename columns
names(SN_inputs)[names(SN_inputs) == "DayOfYear"] <- "Day"
names(SN_inputs)[names(SN_inputs) == "Interp.Value"] <- "GW_pCO2_ppm"
names(SN_inputs)[names(SN_inputs) == "LowerBound"] <- "GW_pCO2_ppm_lower"
names(SN_inputs)[names(SN_inputs) == "UpperBound"] <- "GW_pCO2_ppm_upper"
names(SN_inputs)[names(SN_inputs) == "NEP_median"] <- "NEP_g_CO2_m2day"
#names(SN_inputs)[names(SN_inputs) == "NEP"] <- "NEP_g_CO2_m2day" #to compare daily estimates 
names(SN_inputs)[names(SN_inputs) == "discharge_modeled"] <- "discharge_modeled_m3s"


SN_inputs$date <- as.Date(SN_inputs$date)

SN_inputs$date_numeric <- as.numeric(SN_inputs$date)
...
if (file.exists(gw_inputs_file)) {
  ...
  gw_inputs <- NULL  # Assign NULL or another placeholder if the file doesn't exist
}

if (discharge_type %in% c("measured")) {
  SN_inputs <- SN_inputs[!is.na(SN_inputs$measured_discharge), ]
} 
...
stopifnot(inherits(dem, "RasterLayer"))
dem <- raster::readAll(dem)              
stopifnot(!is.na(raster::crs(dem)))      # DEM must have a defined CRS
...
# # libraries 
rm(list=ls()[! ls() %in% c("month","map", "lines", "stream_reach", "catchment", "PI", "dem", "SN_inputs", "slope", "aspect", "hill", "gw_inputs", "site", "run_type", "discharge_type", "gw_multiplier", "k600_multiplier", "tm_inputs", "threshold_run")])
library(raster)
library(geosphere)
library(fields)
library(dplyr)
...
unique_dates <- sort(unique(SN_inputs$date_numeric))

To <- tm_inputs$Ksat*tm_inputs$Ksat_mult/tm_inputs$f
...
SN_inputs_subset <- SN_inputs[SN_inputs$date_numeric == date, ] 


# Check if SN_inputs has no rows
if (SN_inputs_subset$measured_discharge == 0) {
...
ERQ <- SN_inputs_subset$measured_discharge #m3/s #using modeled 

#create data for model 
SN_geom <- map

#determine what stream lines to run based on top model if fun type is not full 
if (run_type != "full") {
  median_km <- median(stream_reach$LengthKM, na.rm = TRUE)
  top_model <- catchment %>%
    dplyr::select(Permanent_Identifier, d_area) %>%
    left_join(
      map %>%
        group_by(Permanent_Identifier) %>%
        summarise(
          slope = median(slope, na.rm = TRUE),
          Q = median(Q_multiplier, na.rm = TRUE) * ERQ,
          .groups = "drop"
        ),
      by = "Permanent_Identifier"
    ) %>%
    left_join(
      stream_reach %>%
        dplyr::select(Permanent_Identifier, LengthKM),
      by = "Permanent_Identifier"
    ) %>%
    mutate(
      zi = -1 / tm_inputs$f * log((Q * 3600 * 24 / (100)) / (slope * To))
    )
...
valid_ids <- top_model %>%
  filter(zi <= 0) %>%
  pull(Permanent_Identifier)

# Filter table and names accordingly
SN_geom <- SN_geom %>%
  filter(Permanent_Identifier %in% valid_ids)
...
if (threshold_run == "TRUE") {
  SN_geom$Q_threshold <- SN_geom$Q_multiplier * ERQ
...
# Subset points 
SN_geom <- SN_geom[complete.cases(SN_geom$coords.x1, SN_geom$coords.x2), ]
SN_geom$coords.x1 <- as.numeric(SN_geom$coords.x1)
SN_geom$coords.x2 <- as.numeric(SN_geom$coords.x2)
SN_geom <- SN_geom[is.finite(SN_geom$coords.x1) & is.finite(SN_geom$coords.x2), ]
row.names(SN_geom) <- NULL

# Make SpatialPointsDataFrame and align CRS with the DEM
sp::coordinates(SN_geom) <- ~ coords.x1 + coords.x2
...
nep <- switch(
  run_type,
  "regular" = SN_inputs_subset$NEP_g_CO2_m2day,          # Regular run
  "full"= SN_inputs_subset$NEP_g_CO2_m2day,          # Full run
  "gw_inputs" = SN_inputs_subset$NEP_g_CO2_m2day,          # gw inputs run
  "lower" = SN_inputs_subset$NEP_lower, #lower bound 
  "upper" = SN_inputs_subset$NEP_upper,      # Upper bound
  "max_gw" = SN_inputs_subset$NEP_g_CO2_m2day,
  "NEP_daily" = SN_inputs_subset$daily_NEP, #daily NEP  
  "NEP_upper" = SN_inputs_subset$NEP_max,  #maximum NEP observed 
  "gw_upper" = SN_inputs_subset$NEP_g_CO2_m2day
)

#Cgw subset for the day of year 
groundwater <- switch(
  run_type,
  "regular" = SN_inputs_subset$GW_pCO2_ppm,          # Regular run
  "full" = SN_inputs_subset$GW_pCO2_ppm,          # Full run
  "gw_inputs" = SN_inputs_subset$GW_pCO2_ppm,          # Full run
  "lower" = SN_inputs_subset$GW_pCO2_ppm_lower,      # Lower bound
  "upper" = SN_inputs_subset$GW_pCO2_ppm_upper,      # Upper bound
  "max_gw" = SN_inputs_subset$gw_max, #max_gw concentration 
  "NEP_daily" = SN_inputs_subset$GW_pCO2_ppm,
  "NEP_upper" = SN_inputs_subset$GW_pCO2_ppm,
  "gw_upper" = SN_inputs_subset$gw_max
)

groundwater <- groundwater * gw_multiplier

#slope from map dataframe - calculated with dem 
slope <- as.data.frame(SN_geom$slope) 
...
#e <- extract(dem, coordinates(SN_geom)[,-3])
e <- raster::extract(dem, SN_geom)
### MAKE A TABLE OF COORDINATES, ELEVATIONS
table <- coordinates(SN_geom)[,-3]
table <- cbind(table, e)

### EXTRACT STREAM REACH ID'S AND ADD TO OVERALL TABLE
plusId <- array(dim = length(SN_geom$Permanent_Identifier))
for(i in 1:length(plusId)){
  plusId[i] <- toString(SN_geom$Permanent_Identifier[i])
}
...
for (i in 1:length(ids)) {
  ...
  if (nrow(reach) > 1) {
    ...
    de <- c(NA, diff(reach[, 3]))  # First difference is NA since there's no previous point
    # Calculate distances between successive points using distGeo for vectorized operation
...
# Assign AreaSqKm based on the matched indices
...
temp <- as.data.frame(SN_inputs_subset$AvgWaterTemp_C)
temp <- as.data.frame(median(SN_inputs$AvgWaterTemp_C,na.rm = TRUE)) #for median measurement test 
...
atm <- SN_inputs_subset$atm_CO2_ppm #ppm 
#atm <- median(SN_inputs$atm_CO2_ppm,na.rm=TRUE) #ppm  #for median measurement test 
...
for(i in 1:length(names)){
  ...
  #  k_600[j] <- 970 * l[[i]][j, 5]^0.798 * velocity[j]^0.895
#}
  k_600[j] <-  ((velocity[j]*l[[length(l)]][j,5])^0.89*depth[j]^0.54*5037) 
  ...
  if (l[[i]][j, 5] < 0.02) {
    ...
    k_600[j] <- 970 * l[[i]][j, 5]^0.798 * velocity[j]^0.895
      ...
      k_600[j] <- k_600[j]*k600_multiplier
 ...
 c_back_3[j] <-  (c_back_2[j-1] +( q[j] - q[j-1] )/velocity[j]*1/(width[j]*depth[j])*C_gw +  l[[i]][j,8]/velocity[j]*nep/depth[j]/1000)/(1+( q[j] - q[j-1] )/velocity[j]*1/(width[j]*depth[j])+l[[i]][j,8]/velocity[j]*k_co2[j])
  ...
  ### ALL THE OTHER STREAM SEGMENTS NEED TO CALCULATE Q, AND CO2 BASED ON MASS-WEIGHTED AVERAGE OF CONTRIBUTING STREAM SEGMENTS
    ...
    area <- array(sum(c(ar, table[which(table[,4] == names[i])[1],7]), na.rm = T), dim = length(area)) # set area equal to total contributing area -- will sum through iterations so bottom stream reach is the total area 
  ...
  #  k_600[k] <- 970 * l[[i]][k, 5]^0.798 * velocity[k]^0.895
  #}
     k_600[k] <-  ((velocity[k]*l[[length(l)]][k,5])^0.89*depth[k]^0.54*5037) 
  ...
  k_600[k] <- k_600[k]*k600_multiplier 
      ####
...
# Store the resulting dataframe in the list
  ...
  #free up memory
  if ((which(unique_dates == date) %% 10) == 0) {
...
summary_df <- data.frame(
  ...
  #Percent_first_order_streams = numeric(),
...
for (i in seq_along(result_list)) {
  ...
  if (nrow(df) == 0) {
    ...
    #sensor_pCO2_ppm <- tail(df[nrow(df) - 15, 3], 1)
...
summary_df <- merge(summary_df,SN_inputs, by = "date_numeric") 
