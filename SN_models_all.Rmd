...
site <- "CARI" #options are: MART, KING, CARI, COMO, WALK
run_type <- "regular" #options are: regular, full, lower, and upper, gw_inputs (note full should only be used for modeled)
discharge_type <- "measured" #options are measured or modeled (only measured for CARI)

gw_multiplier <- 1.92 #testing how gw offset effects annual emissions 
...
#names(WEPP_inputs)[names(WEPP_inputs) == "NEP"] <- "NEP_g_CO2_m2day" #to compare daily estimates 
...
# # libraries 
rm(list=ls()[! ls() %in% c("month","map", "lines", "stream_reach", "catchment", "PI", "dem", "WEPP_inputs", "WEPP_spatial", "slope", "aspect", "hill", "gw_inputs", "site", "run_type", "discharge_type", "gw_multiplier", "k600_multiplier")])
library(raster)
library(geosphere)
library(fields)
library(dplyr)
...
for(i in 1:length(names)){
  ...
  if (site == "KING" || site == "COMO") {
  # KING and COMO - Ray3_lower
...
for (i in seq_along(result_list)) {
  ...
  if (nrow(df) == 0) {
    ...
    #sensor_pCO2_ppm <- df[nrow(df) - 22, 3]
...
file_suffix <- paste0(gw_mult, k600_mult)
